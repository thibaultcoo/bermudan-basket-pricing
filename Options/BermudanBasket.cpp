...
// Initializing the bermudan instrument object, taking its parameters from the comprehensive Instance built at the beginning
BermudanBasket::BermudanBasket(StochasticProcess* process, const Instance& Inst, int L)
	: Options(process, Inst.getStrike(), Inst.getRates(), Inst.getMatu()), exerciseDates(Inst.getBermudans()), L(L), weights(Inst.getWeights()), spots(Inst.getSpots()), corrMatrix(Inst.getVariances()) { }

// Bermudan instrument pricing methodology
...
// Price methodology considering an antithetic variable in the simulation
...
// Price methodology using a control variate
...
double BermudanBasket::priceQuasiMC(int numSimulations)
{
	int numExerciseDates = exerciseDates.size();
	Eigen::MatrixXd basketValues(numSimulations, numExerciseDates);
	Eigen::MatrixXd exerciseTimes(numSimulations, numExerciseDates);
